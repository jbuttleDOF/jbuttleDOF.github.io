...
dsv("/DRUpopPyramidXhatch/data/caprojdata5yr_1970-2070.csv", function(csv){
	...
	*********************************************************************************/
	...
	d3.json('/DRUpopPyramidXhatch/data/cb_2014_us_county_5m.json', function(error, ca1) {
...
/*
	...
	// var fillPattern = svgb.append("pattern")
	// 	.attr("id", "rectpattern")
	...
	defs = svgb.append('defs');

	// BACKWARD SLASH LINE - RED
	pattern = defs
		.append('pattern')
	...
	/* pattern2.append('rect')
            .attr('width', 2)
            .attr('height', 4)
            .attr('fill', "rgba(253,184,30,0.75)");
			// .attr('fill', "rgba(255,255,255,1)"); */

	/* pattern2.append('circle')
            .attr("cx", 2)           // Center the dot in the tile
            .attr("cy", 2)
	...
	pattern3 = defs
		.append("pattern")
			.attr("id", "backward-slash-hatch") // Unique ID for the pattern
    		.attr("patternUnits", "userSpaceOnUse")
			.attr("patternTransform", "rotate(-45)")
			.attr("width", 4)
			.attr("height", 4);
	...
	groups = svgb.selectAll('.layers')
		...
		.append("g")
		.attr("class", "layers")		
		.style("fill", function(d,i) {return fillr[i]; });
	...
	svgb.append("rect")
		...
		.style("fill", function(d) {return fillr[0]; });
	...
	svgb.append("rect")
		...
		.style("fill", function(d) {return fillr[1]; });
	...
	svgb.append("rect")
		...
		.style("fill", function(d) {return fillr[2]; });
...
(function () { // script to recalulate content contains when window is resized
  var resizeTimer;
